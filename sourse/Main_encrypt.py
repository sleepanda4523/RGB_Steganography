...
def main():
     ...
     bintext = File_bin()
     #cip = RSA(bintext)
     rgbstring,last_len = rgbtext(bintext)
     rgb_png(rgbstring)
...
def test(e, d, n, plaintext):       # 암호화 진행 + 다시 복호화해 암호화가 정상적으로 진행되는지 test
    ...
    for char in cip:
        cip_str+=char[2:]
    return cip_str, decrypt(d, n, cip_str)
